...
static int VehPhysCrash_ShouldPlayLocalFeedback(struct Driver *driver, struct Driver *otherDriver)
{
#if defined(__vita__)
	if (NativeAdhoc_IsConnected() && (driver != NULL) && (driver->driverID < 2))
	{
		return NativeAdhoc_ShouldPresentInteractionVoice(
			driver->driverID,
	...
	if ((driver1->actionsFlagSet & ACTION_MASK_WEAPON) == 0)
	{
		if ((driver2->actionsFlagSet & ACTION_MASK_WEAPON) != 0)
		{
			...
			if ((canPlayFeedback != 0) && VehPhysCrash_ShouldPlayLocalFeedback(driver1, driver2) && (driver1->kartState != KS_BLASTED) && (driver1->invincibleTimer == 0))
			{
				...
				Voiceline_RequestPlayDriver(VEH_PHYS_CRASH_VOICELINE_CRASH, driver1->driverID, VEH_PHYS_CRASH_VOICELINE_PRIORITY);
			}
		}
                                           ...
                                           u32 canPlayFeedback)
{
	...
	if ((selfThread->modelIndex == DYNAMIC_PLAYER) || (otherThread->modelIndex == DYNAMIC_PLAYER))
	{
		...
		if ((canPlayFeedback != 0) && VehPhysCrash_ShouldPlayLocalFeedback(selfDriver, otherDriver) && (selfDriver->kartState != KS_BLASTED) &&
		    (selfDriver->invincibleTimer == 0) && (otherDriver->kartState != KS_BLASTED) && (otherDriver->invincibleTimer == 0))
		{
			...
			{
				Voiceline_RequestPlayDriver(VEH_PHYS_CRASH_VOICELINE_HARD_CRASH, selfDriver->driverID, VEH_PHYS_CRASH_VOICELINE_PRIORITY);
			}
		}
	}
